...
>>>>>>> 4939321 (feat: Add smart task suggestions and one-click task breakdown)
    const breakDownTask = (task) => {
        ...
        // Basic heuristics for splitting
        if (titleLower.includes('clean') || titleLower.includes('tidy')) {
            subtasks = ["Clear surfaces", "Wipe down", "Vacuum/Sweep", "Take out trash"];
        } else if (titleLower.includes('dev') || titleLower.includes('code') || titleLower.includes('build')) {
            subtasks = ["Read requirements", "Draft plan/architecture", "Write code", "Test locally", "Push changes"];
        } else if (titleLower.includes('meal') || titleLower.includes('cook') || titleLower.includes('dinner')) {
            subtasks = ["Find recipe", "Check ingredients", "Prep ingredients", "Cook", "Clean up"];
        } else if (titleLower.includes('workout') || titleLower.includes('exercise')) {
            subtasks = ["Put on gym clothes", "Stretch/Warmup", "Main routine", "Cool down", "Shower"];
        } else {
            // Generic breakdown
            subtasks = ["Define first step", "Gather materials needed", "Execute main work", "Review & finish up"];
        }

        // Add subtasks
        subtasks.forEach((subTitle, index) => {
            state.tasks.push({
                id: Date.now().toString() + Math.random().toString(36).substring(7) + index,
                title: `${task.title} - ${subTitle}`,
                categoryId: task.categoryId,
                date: task.date,
                time: task.time,
                completed: false,
        ...
        // Mark original task as completed or delete it? Let's delete the original since it's now broken down.
        deleteTask(task.id);
    };
...
>>>>>>> 4939321 (feat: Add smart task suggestions and one-click task breakdown)
            ...
            breakdownBtn.className = 'breakdown-task-btn';
            breakdownBtn.innerHTML = '⚡';
            breakdownBtn.title = "Break down into steps";
...
>>>>>>> 4939321 (feat: Add smart task suggestions and one-click task breakdown)
    ...
    const renderSuggestions = () => {
        ...
        // Find tasks that were completed in the past, but don't exist today
        const completedPastTasks = state.tasks.filter(t => t.completed && t.date !== todayStr);

        // Count frequencies of completed tasks to find habits
        const taskFrequencies = {};
        completedPastTasks.forEach(t => {
            if (!taskFrequencies[t.title]) {
                taskFrequencies[t.title] = { count: 0, categoryId: t.categoryId };
            }
            taskFrequencies[t.title].count += 1;
        });

        const suggestions = [];

        // Add frequent tasks not scheduled for today
        Object.keys(taskFrequencies).forEach(title => {
            const isToday = state.tasks.some(t => t.title.toLowerCase() === title.toLowerCase() && t.date === todayStr);
            if (!isToday && taskFrequencies[title].count >= 2) { // Suggested if done at least twice before
                suggestions.push({
                    title: title,
                    categoryId: taskFrequencies[title].categoryId,
                    reason: 'Frequent'
                });
            }
        });
        ...
        if (topSuggestions.length === 0) {
            suggestionList.innerHTML = '<li style="color: var(--text-muted);">No suggestions right now. Keep completing tasks!</li>';
            return;
        }

        topSuggestions.forEach(sugg => {
            const li = document.createElement('li');
